...
1. **Double-click `Dolly.bat`** → installs deps on first run, starts the server, and
...
3. In the **CS2 console**, load the bridge:
   ```
   mirv_script_load "C:\Users\User\Downloads\cs2-dolly\bridge\dolly-bridge.js"
   ```
...
> `start-server.bat` console (if open) before launching `Dolly.bat`.
...
├─ start-server.bat     ← double-click to launch the editor server
├─ bridge/
│  └─ dolly-bridge.js  ← runs inside CS2 via HLAE (mirv_script_load)
├─ server/
...
```
┌──────────────────────────────┐      WebSocket (localhost)       ┌───────────────────────────┐
│  Editor (this repo)          │  ◀── cam pose, demo tick/time ── │  HLAE bridge (dolly-       │
│  Phase 1: Node console        │                                  │  bridge.mjs) running inside │
│  Later:   Tauri + React/TS    │  ─── exec, keyframe ops ──────▶  │  CS2 via mirv-script        │
└──────────────────────────────┘                                  └───────────────────────────┘
```

- **bridge/dolly-bridge.js** — runs *inside* CS2 through HLAE (`mirv_script_load`).
...
```powershell
cd C:\Users\User\Downloads\cs2-dolly\server
npm install
npm start
```
